...
export default function CosmicChainV11() {
  ...
  const infinityWave = (root) =>
    Math.abs(Math.sin(tick * root.freq + root.phase)) * root.infinity;

  const infinityMatrix = chain.map((a) =>
    chain.map((b) =>
      (infinityWave(a) * infinityWave(b) * ((a.energy + b.energy) / 2)).toFixed(2)
    )
  );

  const infinityBranches = chain.map((root) => ({
    id: root.id,
    totality: (root.infinity * infinityWave(root)).toFixed(2)
  }));

  const topInfinity = infinityBranches.reduce((a, b) =>
    parseFloat(a.totality) > parseFloat(b.totality) ? a : b
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{
          color: chain.find(c => c.id === topInfinity.id).color
        }}>
