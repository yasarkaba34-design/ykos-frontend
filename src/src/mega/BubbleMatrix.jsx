...
const bubbleData = [
  { id: "OK", color: "#FFD700", label: "OK [K]", meaning: "Yön, Yükseliş, Bağlantı, Akış", depth: 0.0942 },
  { id: "AT", color: "#00BFFF", label: "AT [T]", meaning: "Hareket, Güç, İtki, Başlangıç", depth: 0.0875 },
  { id: "ER", color: "#8A2BE2", label: "ER [T]", meaning: "İnsan, Bilinç, Eylem, Ruh", depth: 0.0913 },
  { id: "EL", color: "#00FF7F", label: "EL [T]", meaning: "El, Yaratım, Dokunuş, İnşa", depth: 0.0897 },
];

export default function YKOSBubbleMatrix() {
  return (
    <div className="bubble-matrix">
      {bubbleData.map((b) => (
        <div
          ...
          style={{
            backgroundColor: b.color,
            boxShadow: `0 0 ${b.depth * 100}px ${b.color}`,
            animation: `pulse ${2 + b.depth * 10}s infinite`,
          }}
        >
          <span className="bubble-label">{b.label}</span>
          <div className="bubble-info">
            <p>{b.meaning}</p>
            <p className="depth">YKOS Uyum Derinliği: {b.depth.toFixed(4)}</p>
          </div>
        </div>
      ))}
    </div>
...
const bubbleData = [
  { id: "OK", color: "#FFD700", label: "OK [K]", meaning: "Yön, Yükseliş, Bağlantı, Akış", depth: 0.0942, resonance: 0.82 },
  { id: "AT", color: "#00BFFF", label: "AT [T]", meaning: "Hareket, Güç, İtki, Başlangıç", depth: 0.0875, resonance: 0.74 },
  { id: "ER", color: "#8A2BE2", label: "ER [T]", meaning: "İnsan, Bilinç, Eylem, Ruh", depth: 0.0913, resonance: 0.91 },
  { id: "EL", color: "#00FF7F", label: "EL [T]", meaning: "El, Yaratım, Dokunuş, İnşa", depth: 0.0897, resonance: 0.77 },
];
<div
  key={b.id}
  className="bubble"
  style={{
    backgroundColor: b.color,
    boxShadow: `0 0 ${b.depth * 100}px ${b.color}`,
    animation: `pulse ${2 + b.depth * 10}s infinite, vibrate ${0.5 + (1 - b.resonance)}s infinite`,
  }}
>
export default function YKOSBubbleMatrix() {
  return (
      ...
      {bubbleData.map((b) => (
        ...
        >
          ...
          <div className="bubble-info">
            <p>{b.meaning}</p>
            <p className="depth">YKOS Uyum Derinliği: {b.depth.toFixed(4)}</p>
            <p className="resonance">Rezonans: {b.resonance}</p>
          </div>
        </div>
      ))}
    </div>
...
<div
  ...
  style={{
    ...
    animation: `pulse ${2 + b.depth * 10}s infinite, vibrate ${0.5 + (1 - b.resonance)}s infinite`,
    position: "absolute",
    left: `${atlasCoords[b.id].x}px`,
    top: `${atlasCoords[b.id].y}px`,
  }}
>
export default function YKOSBubbleMatrix() {
  return (
    <div className="bubble-matrix atlas-map">
      {bubbleData.map((b) => (
        <div
          key={b.id}
          className="bubble"
...
function drawFlowLines(ctx) {
  flowLines.forEach(line => {
    ...
    ctx.beginPath();
    ctx.moveTo(start.x + 60, start.y + 60); // balon merkezleri
    ctx.lineTo(end.x + 60, end.y + 60);
...
export default function YKOSBubbleMatrix() {
  ...
  return (
    <div className="bubble-matrix atlas-map">
      <canvas id="flowCanvas" width={1000} height={600} className="flow-canvas"></canvas>
...
useEffect(() => {
  ...
  if (!canvas) return;
  const ctx = canvas.getContext("2d");
  setTimeout(() => drawFlowLines(ctx), 300); // render sonrası çizim
}, []);
/**
 * BubbleMatrix.jsx
 * - Kök hece baloncuklarını gösterir
 * - RMV vektörüne göre ilgili baloncukları parlatır
 */

import { useEffect, useState } from "react";
import "./BubbleMatrix.css";

export default function BubbleMatrix({ rmv }) {
  const [activeRoots, setActiveRoots] = useState([]);

  useEffect(() => {
    setActiveRoots(rmv); // RMV geldiğinde aktif kökleri güncelle
  }, [rmv]);
  ...
  return (
    <div className="bubble-matrix">
      {roots.map((root) => (
        <div
          key={root.id}
          className={`bubble ${activeRoots.includes(root.id) ? "active" : ""}`}
